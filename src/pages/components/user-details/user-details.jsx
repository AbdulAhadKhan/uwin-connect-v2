...
function UserAvatar({ imageID, id }) {
    ...
    const [previewImage, setPreviewImage] = useState(imageID ? generateImageLink(imageID) : "")
    ...
    return (
        <div className="avatar-container">
            <div className="user-details__avatar" onMouseEnter={() => setMouseOver(true)} 
            onMouseLeave={() => setMouseOver(false)} onClick={handleClick}>
                    {mouseOver && <div className="avatar-overlay" >
                        <input type="file" id="avatar" name="avatar" accept="image/*" 
                        style={{display: "none"}} ref={inputRef} onChange={handleImageChange} />
                        <h1 className="">Change Image</h1>
                    </div>}
                    { imageID ? <img src={previewImage} alt="User Avatar" /> 
